...
export function cardClick(movie) {
	...
	const movieDetailsLink = getMovieLink(movie.id, "");
	const movieCreditsLink = getMovieLink(movie.id, "/credits");
	const movieProvidersLink = getMovieLink(movie.id, "/watch/providers");

	var movieDetails = sessionStorage.getItem(`detailsData${movie.id}`);
	var movieCredits = sessionStorage.getItem(`creditsData${movie.id}`);
	var movieProviders = sessionStorage.getItem(`providersData${movie.id}`);

	if (movieDetails != null && movieCredits != null && movieProviders != null) {
		var detailsData = JSON.parse(movieDetails);
		var creditsData = JSON.parse(movieCredits);
		var providersData = JSON.parse(movieProviders);
		...
		Promise.all([fetch(movieDetailsLink), fetch(movieCreditsLink), fetch(movieProvidersLink)])
			...
			.then(([detailsData, creditsData, providersData]) => {
				sessionStorage.setItem(`detailsData${movie.id}`, JSON.stringify(detailsData));
				sessionStorage.setItem(`creditsData${movie.id}`, JSON.stringify(creditsData));
				sessionStorage.setItem(`providersData${movie.id}`, JSON.stringify(providersData));
...
document.addEventListener(
	...
	function (event) {
		...
		if (details.classList.contains("d-block") && !details.contains(event.target) && !event.target.closest(".card")) {
			closeClick();
		}
	},
	true
